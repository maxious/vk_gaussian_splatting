 ...
 * 
 * @return         vector containing the sparse voxel octree
 */
std::vector<uint8_t> encode_sparse_voxel_octree_cpu(
    const std::vector<uint32_t>& codes,
    const uint32_t depth
) {
    size_t N_leaf = codes.size();
    ...
    for (size_t i = 0; i < N_leaf; i++) {
        ...
        for (uint32_t j = insert_from; j < depth; j++) {
            ...
            if (j > insert_from) {
                svo.push_back(0);
                stack_ptr[j] = (uint32_t)svo.size()-1;
            }
            // Update parent pointers
            svo[stack_ptr[j]] |= (1 << insert_stack[j]);
            // Update stack
            if (j < depth-1) {
                stack[j] = (uint8_t)insert_stack[j];
            }
        }
    }
 ...
 * @param depth    The depth of the sparse voxel octree
 * 
 * @return         [N] vector containing the morton codes
