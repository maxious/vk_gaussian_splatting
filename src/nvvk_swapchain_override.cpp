...
VkResult nvvk::Swapchain::initResources(VkExtent2D& outWindowSize, bool vSync)
{
  ...
  // NOTE: Original nvvk::Swapchain might not have m_colorSpace.
  // We use our side-channel to store it.
  HDRSupport::setGlobalColorSpace(surfaceFormat2.surfaceFormat.colorSpace);
  HDRSupport::setGlobalFormat(m_imageFormat);

  // If m_colorSpace exists in the header (we can't know for sure without parsing it or trial compile), 

  // we might try to set it, but to be safe and avoid memory corruption if it doesn't exist 
  // (and we are linking against objects that think it doesn't), we skip it.
  // The user asked to avoid modifying nvpro_core2, so we assume the clean header.
