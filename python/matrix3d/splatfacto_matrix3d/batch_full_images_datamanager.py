...
@dataclass
class BatchFullImageDatamanagerConfig(FullImageDatamanagerConfig):
    ...
    max_num_iterations: int = 15000
    """The number of training iterations."""
    num_image_each_iteration: int = 10
    """The number of images used for training in each iteration."""
...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Matrix3D add-on code
        # find captured images
        self.captured_images_indices = []
        for idx, p in enumerate(self.train_dataset.image_filenames):
            if 'ref' in str(p):
                self.captured_images_indices.append(idx)
        if len(self.captured_images_indices) > 0:
            # compute distance all cameras to captured images
            cameras = self.train_dataset.cameras
            captured_camera_pos = torch.cat([cameras[i].camera_to_worlds[:, 3][None] for i in self.captured_images_indices])
            all_camera_pos = torch.cat([cameras[i].camera_to_worlds[:, 3][None] for i in range(len(self.train_dataset.image_filenames))])
            self.min_camera_dists = (captured_camera_pos.unsqueeze(0) - all_camera_pos.unsqueeze(1)).norm(dim=-1).min(dim=-1)[0]
            self.min_camera_dists[self.captured_images_indices] = -1  # set captured camera flag == -1
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        if self.config.num_image_each_iteration > 1:
            if len(self.train_unseen_cameras) < self.config.num_image_each_iteration:
                self.train_unseen_cameras = self.sample_train_cameras()
            # image_idx = [self.train_unseen_cameras.pop(0) for _ in range(self.config.num_image_each_iteration)]
            image_idx = self.captured_images_indices + [self.train_unseen_cameras.pop(0) for _ in range(self.config.num_image_each_iteration-1)]
        else:
        ...
        if isinstance(image_idx, int):
            ...
            data["camera_dist"] = self.min_camera_dists[image_idx].to(self.device)
            data["max_num_iterations"] = torch.Tensor([self.config.max_num_iterations]).to(self.device)
            ...
            if camera.metadata is None:
                camera.metadata = {}
            camera.metadata["cam_idx"] = torch.Tensor([image_idx]).long()
        elif isinstance(image_idx, list):
            data_list = [self.cached_train[idx] for idx in image_idx]
            data = {}
            for key in data_list[0].keys():
                if key == "image" or key == "mask":
                    data[key] = torch.cat([d[key].unsqueeze(0) for d in data_list])
                elif key == "image_idx":
                    data[key] = torch.tensor([d[key] for d in data_list], dtype=torch.long)
                else:
                    data[key] = [d[key] for d in data_list]
            data["camera_dist"] = self.min_camera_dists[image_idx].to(self.device)
            data["max_num_iterations"] = torch.Tensor([self.config.max_num_iterations]).to(self.device)
            assert len(self.train_cameras.shape) == 1, "Assumes single batch dimension"
            camera = self.train_cameras[torch.Tensor(image_idx).long()].to(self.device)
            if camera.metadata is None:
                camera.metadata = {}
            camera.metadata["cam_idx"] = torch.Tensor(image_idx).long()            
