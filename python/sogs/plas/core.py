...
def params_to_blocky(params, block_size, block_divisor, num_pixel_blocks, shift_y, shift_x):
    # params: (c, h, w)
    ...
    params_unshuffled_blocky_first = params_unshuffled_blocky_inline.permute(0, 2, 4, 1, 3, 5)
...
def blocky_to_params(
    ...
    # (r^2, c, pb, h/r/pb, pb, w/r/pb)
    params_unshuffled_blocky_inline = params_unshuffled_blocky_first.permute(0, 3, 1, 4, 2, 5)
...
def reorder_blocky_shuffled(
    ...
    C = squared_l2_distance_with_batch_dim(blockwise_shuffled_params, blockwise_shuffled_target)
    ...
    shuffled_block_indices_cand_exp = shuffled_block_indices_cand.expand_as(best_perm_indices)

    # best_positions: (num_blocks, solver_groups, 4)
    best_positions = torch.gather(shuffled_block_indices_cand_exp, -1, best_perm_indices)
    ...
    best_grid_indices = torch.gather(grid_indices_blocky_flat, -1, best_positions_flat.unsqueeze(1))
...
def reorder_plas(
    ...
    target = low_pass_filter(params, filter_size_x, filter_size_y, border_type_x, border_type_y)
