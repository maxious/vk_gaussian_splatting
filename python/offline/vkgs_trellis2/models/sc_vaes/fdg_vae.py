...
class FlexiDualGridVaeDecoder(SparseUnetVaeDecoder):
    ...
    def forward(self, x: sp.SparseTensor, gt_intersected: sp.SparseTensor = None, **kwargs):
        if not O_VOXEL_AVAILABLE:
            raise RuntimeError(
                "o_voxel is required for FlexiDualGridVaeDecoder but not available. "
                "This module requires CUDA. Use CUDA environment for mesh conversion."
            )
        decoded = super().forward(x, **kwargs)
        if self.training:
            h, subs_gt, subs = decoded
            vertices = h.replace(
                (1 + 2 * self.voxel_margin) * F.sigmoid(h.feats[..., 0:3]) - self.voxel_margin
            )
