...
class Trellis2TexturingPipeline(Pipeline):
    ...
    def postprocess_mesh(
        ...
        mask = mask.cpu().numpy()
        base_color = np.clip(
            attrs[..., self.pbr_attr_layout["base_color"]].cpu().numpy() * 255, 0, 255
        ).astype(np.uint8)
        metallic = np.clip(
            attrs[..., self.pbr_attr_layout["metallic"]].cpu().numpy() * 255, 0, 255
        ).astype(np.uint8)
        roughness = np.clip(
            attrs[..., self.pbr_attr_layout["roughness"]].cpu().numpy() * 255, 0, 255
        ).astype(np.uint8)
        alpha = np.clip(
            attrs[..., self.pbr_attr_layout["alpha"]].cpu().numpy() * 255, 0, 255
        ).astype(np.uint8)
