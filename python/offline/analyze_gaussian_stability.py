...
from offline.motion_tracking_cpu import match_gaussians_faiss as match_gaussians_bidirectional
from offline.ply_io import load_static_gaussian_ply
from offline.types import GaussianFrame
...
def main():
    ...
    parser.add_argument(
        "--input", "-i", type=Path, required=True, help="Directory containing per-frame PLY files"
    )
    parser.add_argument(
        "--pattern", type=str, default="frame_*.ply", help="Glob pattern for PLY files"
    )
    parser.add_argument(
        "--max-distance", type=float, default=0.05, help="Max distance for position matching"
    )
    parser.add_argument("--max-frames", type=int, default=None, help="Max frames to analyze")
