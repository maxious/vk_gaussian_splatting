...
def match_gaussians_batch_faiss(
    ...
    More efficient than calling match_gaussians_faiss repeatedly
    because it reuses resources.
    """
    import faiss
    ...
    use_gpu = use_gpu and check_faiss_gpu_available()
    ...
    for i in range(len(all_means) - 1):
        ...
        logger.debug(f"Frame {i}->{i+1}: {len(matches)} matches ({100*len(matches)/len(means_a):.1f}%)")
...
def build_trajectories_gpu(
    ...
    Uses union-find on CPU (fast enough) with FAISS for matching.
    FAISS GPU is used if available, otherwise CPU.
    """
    if len(frames) == 0:
    ...
    if check_faiss_available():
        use_gpu = check_faiss_gpu_available()
        logger.info(f"Using FAISS {'GPU' if use_gpu else 'CPU'} for matching")
        all_matches = match_gaussians_batch_faiss(all_means, max_distance, use_gpu=use_gpu)
    else:
        logger.info("FAISS not available, using scipy KDTree")
...
def fit_trajectories_cupy(
    ...
    logger.info(f"Motion stats: velocity magnitude mean={vel_mag.mean():.4f}, max={vel_mag.max():.4f}")
...
def compute_motion_vectors_gpu(
    ...
    if check_cuda_available():
        ...
        return (all_means, all_scales, all_rotations, all_colors, all_opacities,
                all_motion, all_time_center, all_time_scale)
