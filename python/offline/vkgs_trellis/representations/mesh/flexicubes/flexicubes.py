...
# Mock check_tensor to avoid kaolin dependency for non-mesh outputs
def check_tensor(tensor, shape, throw=True):
    """Mock function - just checks basic tensor properties."""
    ...
    if shape is not None and len(shape) > 0:
        ...
        if len(actual_shape) != len(shape):
            ...
            return False
        for i, (a, e) in enumerate(zip(actual_shape, shape)):
...
class FlexiCubes:
    ...
    def __call__(
        ...
        assert torch.is_tensor(cube_idx) and check_tensor(cube_idx, (None, 8), throw=False), (
        ...
        assert gamma_f is None or (
            torch.is_tensor(gamma_f) and check_tensor(gamma_f, (num_cubes,), throw=False)
    ...
    def _compute_vd(
        ...
        """
        alpha_nx12x2 = torch.index_select(input=alpha, index=self.cube_edges, dim=1).reshape(
            -1, 12, 2
        )
        ...
        if voxelgrid_colors is not None:
            ...
            vd_color = (
                vd_color.index_add_(0, index=edge_group_to_vd, source=uc_group * beta_group)
                / beta_sum
            )
        else:
            vd_color = None
    ...
    def _triangulate(
        ...
        if not training:
            ...
            if vd_color is not None:
                ...
                color_center = (
                    color_02 * gamma_02.unsqueeze(-1) + color_13 * gamma_13.unsqueeze(-1)
                ) / weight_sum.unsqueeze(-1)
