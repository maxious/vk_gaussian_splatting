...
def generate_masks(
    ...
    logger.info(f"Found {len(image_files)} images. Initializing BEN2...")
...
def main():
    parser = argparse.ArgumentParser(description="Generate masks using BEN2")
    ...
    parser.add_argument(
        "--refine",
        action="store_true",
        help="Enable BEN2 foreground refinement (slower but better edges)",
    )
    ...
    generate_masks(
        args.input, args.output, device=args.device, pattern=args.pattern, refine=args.refine
    )
