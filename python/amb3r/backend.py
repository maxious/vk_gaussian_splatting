...
def scatter_mean(src: torch.Tensor, index: torch.Tensor, dim: int = 0) -> torch.Tensor:
    ...
    out = torch.zeros(num_groups, *src.shape[1:], dtype=src.dtype, device=src.device)
    count = torch.zeros(num_groups, dtype=src.dtype, device=src.device)
    ...
    # Broadcast count to match out shape
