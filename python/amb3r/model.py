...
def scale_invariant_alignment(pred, target, mask, trunc=1.0):
    ...
    Returns (aligned_pred, scale).  Operates on the last dim (xyz).
    """
    valid = mask.unsqueeze(-1).expand_as(pred)  # (..., 3)
    pred_v = pred[valid].view(-1, 3)
    tgt_v  = target[valid].view(-1, 3)

    # Solve s = (pred . tgt) / (pred . pred)
    dot_pt = (pred_v * tgt_v).sum()
    dot_pp = (pred_v * pred_v).sum().clamp(min=1e-12)
    scale  = (dot_pt / dot_pp).clamp(min=1e-6, max=1e6)
