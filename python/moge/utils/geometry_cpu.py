...
def normalized_view_plane_uv_cpu(
    width: int, height: int, aspect_ratio: Optional[float] = None
) -> np.ndarray:
    ...
    u = np.linspace(
        -span_x * (width - 1) / width, span_x * (width - 1) / width, width, dtype=np.float32
    )
    v = np.linspace(
        -span_y * (height - 1) / height, span_y * (height - 1) / height, height, dtype=np.float32
    )
    u, v = np.meshgrid(u, v, indexing="xy")
    return np.stack([u, v], axis=-1)


def depth_to_points_cpu(depth: np.ndarray, intrinsics: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    y, x = np.meshgrid(
        np.linspace(0, height - 1, height), np.linspace(0, width - 1, width), indexing="ij"
    )
...
def points_to_normals_cpu(
    points: np.ndarray, mask: np.ndarray = None
