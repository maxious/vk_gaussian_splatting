...
def main(share: bool, pretrained_model_name_or_path: str, model_version: str, use_fp16: bool):
    ...
    def run_with_gpu(
        image: np.ndarray, resolution_level: int, apply_mask: bool
    ) -> Dict[str, np.ndarray]:
        image_tensor = (
            torch.tensor(
                image,
                dtype=torch.float32 if not use_fp16 else torch.float16,
    ...
    def run_with_cpu(
        ...
        image_tensor = (
            torch.tensor(image, dtype=torch.float32, device=torch.device("cpu")).permute(2, 0, 1)
            / 255
        )
        output = model.infer(
            image_tensor, apply_mask=apply_mask, resolution_level=resolution_level, use_fp16=False
        )
        output = {k: v.cpu().numpy() for k, v in output.items()}
        return output
    ...
    def run(
        ...
        resolution_level_int = {"Low": 0, "Medium": 5, "High": 9, "Ultra": 30}.get(
            resolution_level, 9
        )
        # 判断当前模型设备
        ...
        points, depth, mask, normal = (
            output["points"],
            output["depth"],
            output["mask"],
            output.get("normal", None),
        )
        ...
        results = {**output, "mask_cleaned": mask_cleaned, "image": image}
        ...
        vertex_uvs = vertex_uvs * np.array([1, -1], dtype=np.float32) + np.array(
        ...
        cv2.imwrite(
            str(output_path / "depth.exr"),
            depth.astype(np.float32),
            [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT],
        )
        cv2.imwrite(
            str(output_path / "points.exr"),
            cv2.cvtColor(points.astype(np.float32), cv2.COLOR_RGB2BGR),
            [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT],
        )
        if normal is not None:
            cv2.imwrite(
                str(output_path / "normal.exr"),
                cv2.cvtColor(
                    normal.astype(np.float32) * np.array([1, -1, -1], dtype=np.float32),
                    cv2.COLOR_RGB2BGR,
                ),
    ...
    def measure(
        results: Dict[str, np.ndarray], measure_points: List[Tuple[int, int]], event: gr.SelectData
    ):
        ...
        if len(measure_points) == 2:
            ...
            distance = np.linalg.norm(
                results["points"][point1[1], point1[0]] - results["points"][point2[1], point2[0]]
            )
            measure_points = []
...
"""
            ...
            with gr.Column():
                ...
                with gr.Accordion(label="Settings", open=False):
                    ...
                    resolution_level = gr.Dropdown(
                        ...
                        label="Inference Resolution Level",
                        value="High",
                    )
            ...
            with gr.Column():
                with gr.Tabs():
                    ...
                    with gr.Tab("Normal", interactive=hasattr(model, "normal_head")):
                        normal_map = gr.Image(
                            type="numpy", label="Normal Map", format="png", interactive=False
                        )
        ...
        if Path("example_images").exists():
            example_image_paths = sorted(
                list(
                    itertools.chain(
                        *[
                            Path("example_images").glob(f"*.{ext}")
            ...
            examples = gr.Examples(
                examples=example_image_paths, inputs=input_image, label="Examples"
            )
